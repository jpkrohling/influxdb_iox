...
impl Table {
    ...
    pub fn to_arrow(&self, chunk: &Chunk, selection: ChunkSelection<'_>) -> Result<RecordBatch> {
        ...
        self.to_arrow_impl(chunk, &self.to_col_selection(chunk, selection)?)
    }

    pub fn schema(&self, chunk: &Chunk, selection: ChunkSelection<'_>) -> Result<Schema> {
        // translate chunk selection into name/indexes:
        self.schema_impl(&self.to_col_selection(chunk, selection)?)
    }

    fn to_col_selection<'a>(
        &self,
        chunk: &'a Chunk,
        selection: ChunkSelection<'a>,
    ) -> Result<TableColSelection<'a>> {
        match selection {
...
mod tests {
    ...
    fn test_to_arrow_schema_subset() {
        ...
        let selection = ChunkSelection::Some(&["float_field"]);
        let actual_schema = table.schema(&chunk, selection).unwrap();
        let expected_schema = SchemaBuilder::new()
            .field("float_field", ArrowDataType::Float64)
            .build()
